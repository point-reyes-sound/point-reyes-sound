...
export default function LandingPage() {
  ...
  const handleDragStart = (e, element) => {
    ...
    // Create a temporary "ghost" element that looks like our fuzzy 3D ball
    const ghostRadius = Math.max(element.radius * 60, 40); // Scale for visibility
    ...
    ghostDiv.style.top = "-1000px"; // Hide it off-screen
    document.body.appendChild(ghostDiv);

    // Set the custom drag image, centering it on the cursor
    e.dataTransfer.setDragImage(ghostDiv, ghostRadius / 2, ghostRadius / 2);

    // Clean up the DOM node immediately after the drag sequence initializes
          ...
          {reactionZone.length < 2 ? (
             reactionZone.map((el, idx) => (
              <div key={idx} className="chamber-element">
                {/* Render the bouncing 3D sphere inside the chamber (NO TEXT OVERLAY) */}
                <div className="phase-space-canvas">
                  <Canvas camera={{ position: [0, 0, 4.5] }}>
                    {/* Scaling it up slightly for the reaction chamber view */}
